...
class Test_Workload_Results():
    ...
    @pytest.mark.skipif((int(no_cores) < 16 or sgx_mode != '1'), reason="Run only on server machines, when dDirect is disabled")
    ...
    @pytest.mark.examples
    @pytest.mark.skipif((int(no_cores) < 16 or sgx_mode != '0'), reason="Run only on server machines, when dDirect is enabled")
