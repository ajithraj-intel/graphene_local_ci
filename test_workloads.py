...
class Test_Workload_Results():
    ...
    @pytest.mark.skipif(int(no_cores) < 16 and sgx_mode == '1', reason="Run only on servers, not on client machines, due to memory problems")
    ...
    @pytest.mark.examples
    @pytest.mark.skipif(int(no_cores) < 16, reason="Run only on servers, not on client machines, due to memory problems")
